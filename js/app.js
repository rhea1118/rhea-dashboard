...
// ===== 数据源 =====
// 取数优先级（自动回退，确保手机打开网页即更新，无需任何 GitHub 推送）：
//   1) Netlify 云端函数 /api/content（服务器端抓取，最稳）
//   2) 浏览器端实时生成（汇率 API + 新闻 RSS 经代理，零服务端依赖，手机打开即更新）
//   3) GitHub raw / jsDelivr（历史静态文件，兜底）
const GH_DATA = {
  ...
  // 浏览器取 RSS 的代理（依次尝试；rss2json 专为浏览器设计且支持跨域）
  rssProxies: [
    q => 'https://api.rss2json.com/v1/api.json?rss_url=' + encodeURIComponent('https://news.google.com/rss/search?q=' + encodeURIComponent(q) + '&hl=en-US&gl=US&ceid=US:en'),
    q => 'https://api.allorigins.win/raw?url=' + encodeURIComponent('https://news.google.com/rss/search?q=' + encodeURIComponent(q) + '&hl=en-US&gl=US&ceid=US:en'),
    q => 'https://corsproxy.io/?url=' + encodeURIComponent('https://news.google.com/rss/search?q=' + encodeURIComponent(q) + '&hl=en-US&gl=US&ceid=US:en')
  ],
  _contentCache: null, _contentTs: 0, _contentFetching: null, _CACHE_TTL: 30 * 60 * 1000,
  _csCache: null, _csTs: 0, _csFetching: null, _CS_TTL: 3 * 60 * 60 * 1000,
  // ① 云端函数
  async fetchContent() {
    ...
    this._contentFetching = (async () => {
      try {
        ...
        if (resp.ok) { const j = await resp.json(); this._contentCache = j; this._contentTs = Date.now(); return j; }
      } catch (e) {}
      return null;
    })();
    const r = await this._contentFetching; this._contentFetching = null; return r;
  },
  // ② 浏览器端实时生成
  dayOfYear(d) { const s = new Date(d.getFullYear(), 0, 0); return Math.floor((d - s) / 86400000); },
  pick(arr, i) { return arr[i % arr.length]; },
  async fetchRateCS() {
    try {
  ...
  async fetchNewsCS(q) {
    for (const make of this.rssProxies) {
      try {
        const resp = await fetch(make(q));
        if (!resp.ok) continue;
        const ct = resp.headers.get('content-type') || '';
        let items = [];
        if (ct.includes('json')) {
          const j = await resp.json();
          if (j && j.items) items = (j.items || []).map(it => ({ title: it.title || '', source: it.source || '', pubDate: it.pubDate || '' }));
        } else {
          const xml = await resp.text();
          const doc = new DOMParser().parseFromString(xml, 'text/xml');
          doc.querySelectorAll('item').forEach(it => {
            let t = (it.querySelector('title') || {}).textContent || '';
            let s = (it.querySelector('source') || {}).textContent || '';
            const idx = t.lastIndexOf(' - ');
            if (idx > 0) { if (!s) s = t.slice(idx + 3); t = t.slice(0, idx); }
            if (t) items.push({ title: t.trim(), source: (s || '').trim(), pubDate: (it.querySelector('pubDate') || {}).textContent || '' });
          });
        }
  ...
  composeBriefingCS(rate, africa, sa, comp, doy) {
    ...
    const africaText = africa.length ? africa.map(n => n.title + '（来源：' + n.source + '）').join('；') : '暂未抓取到非洲相关新闻，建议手动关注非洲基建/矿业/进口政策动态。';
    const saText = sa.length ? sa.map(n => n.title + '（来源：' + n.source + '）').join('；') : '暂未抓取到南美相关新闻，建议手动关注南美（墨西哥/巴西/智利/秘鲁）关税与工程机械需求。';
    const compText = comp.length ? comp.map(n => n.title + '（来源：' + n.source + '）').join('；') : '暂未抓取到竞品动态，建议手动关注徐工/三一/重汽/陕汽/中联重科海外动作。';
    const topics = '目标客户群体（工程承包商 / 矿主 / 维修厂老板）今日讨论热点：① 中国工程机械在非洲/南美的售后服务与配件供应；② 新兴市场基建项目招标与设备采购需求；③ 中国品牌 vs 欧美日韩品牌性价比对比（来源：Facebook / LinkedIn 行业群组观察）';
    const hotA = africa[0] ? africa[0].title : '非洲基建/矿业需求';
    const hotS = sa[0] ? sa[0].title : '南美关税/需求变化';
    const hotC = comp[0] ? comp[0].title : '竞品海外动作';
    return {
      date: date, generatedAt: new Date().toISOString(), source: '浏览器实时抓取（汇率+新闻 RSS）',
      africa: africaText, southamerica: saText, fx: fx, competitor: compText, topics: topics,
      topicSuggestions: {
        fb1_title: '【现货】HOWO / SHACMAN 原厂斗齿 & 滤芯 — 针对今日非洲矿业热点，矿区老板首选耐磨配件，量大价优，DM 询价 👇',
        fb1_img: '配件实物实拍 + 仓库装箱图（3-4 张），带价格标签', fb1_hot: '非洲：' + hotA,
        fb2_title: '【装柜实拍】今日发运：HOWO 自卸车 + 装载机 → 非洲/南美港口 🚢 中国品牌出海实录',
        fb2_img: '装柜过程视频/照片（15-30 秒），突出整车状态', fb2_hot: '南美：' + hotS,
        li_title: '从今日热点看中国工程机械出海：设备出口之外，服务网络本地化才是决胜点',
        li_img: '数据图表 + 目标市场地图标注，专业商务风格', li_hot: '竞品：' + hotC,
        wa_title: '🔥 今日特价：HOWO 牵引车底盘件全系列，支持非洲/南美直发，私信询价 👋',
        wa_img: '产品图 1 张 + 联系方式水印',
        yt_title: '中国品牌工程机械出海实录：HOWO 卡车在非洲/南美矿区的真实表现',
        yt_img: '封面：矿区实拍作业场景，大标题 + 品牌 Logo', yt_hot: '非洲 + 南美市场热点'
      }
    };
  },
  buildLearningCS(doy) {
    ...
    return {
      date: date,
      english: { content: this.pick(EN_POOL, doy) },
      finance: { content: this.pick(FIN_POOL, doy + 1) },
      chatTips: { content: this.pick(CHAT_POOL, doy + 2) }
    };
  },
  async generateClientSide() {
    if (this._csCache && Date.now() - this._csTs < this._CS_TTL) return this._csCache;
    if (this._csFetching) return this._csFetching;
    this._csFetching = (async () => {
      const doy = this.dayOfYear(new Date());
      const [rate, africa, sa, comp] = await Promise.all([
        this.fetchRateCS(),
        this.fetchNewsCS('construction equipment OR mining Africa 2026'),
        this.fetchNewsCS('truck OR machinery tariff Brazil Chile Mexico 2026'),
        this.fetchNewsCS('SANY OR XCMG OR Sinotruk OR Shacman OR Zoomlion overseas 2026')
      ]);
      const data = { briefing: this.composeBriefingCS(rate, africa, sa, comp, doy), learning: this.buildLearningCS(doy) };
      this._csCache = data; this._csTs = Date.now(); return data;
    })();
    const r = await this._csFetching; this._csFetching = null; return r;
  },
  // 读取简报：云端函数 → 浏览器生成 → GitHub 静态
  async fetchBriefing() {
    const content = await this.fetchContent();
    if (content && content.briefing && content.briefing.africa) return content.briefing;
    const cs = await this.generateClientSide();
    if (cs && cs.briefing && cs.briefing.africa) return cs.briefing;
    const urls = [
      this.RAW_BASE + '/daily-brief.json?t=' + Date.now(),
      this.JSDELIVR_BASE + '/daily-brief.json?t=' + Date.now(),
      'data/daily-brief.json?t=' + Date.now(), 'data/briefing.json?t=' + Date.now()
    ];
    for (const url of urls) { try { const resp = await fetch(url); if (resp.ok) return await resp.json(); } catch (e) {} }
    return null;
  },
  // 读取学习：云端函数 → 浏览器生成 → GitHub 静态
  async fetchLearning() {
    const content = await this.fetchContent();
    if (content && content.learning && content.learning.english) return content.learning;
    const cs = await this.generateClientSide();
    if (cs && cs.learning && cs.learning.english) return cs.learning;
    const urls = [
...
// ===== 学习素材库（按日轮换，保证每日新鲜，无需 AI Key / 服务端） =====
const EN_POOL = [
  ...
  `📚 今日外贸英语\n\n【句子】Could you advise the lead time for this item?\n【释义】这件产品的交货期是多久？\n【场景】客户确认样品/价格后，紧接着必问的就是交货期，直接决定能否赶上销售节点。\n【小技巧】"lead time"（交期/前置期）是外贸核心词。报价时主动附上 lead time 会显得专业："Lead time is 25-30 days after deposit." 也能反向问："When do you need the goods?"`,
  `📚 今日外贸英语\n\n【句子】What's your MOQ for this product?\n【释义】这个产品的最小起订量是多少？\n【场景】客户担心起订量太高时，先问清 MOQ 再谈定制或拆分订单。\n【小技巧】"MOQ"（最小起订量）几乎每封开发信都会遇到。若客户量小，可灵活回应："MOQ is 50 pcs, but we can support a mixed container for first trial." 用混装降低门槛。`,
  `📚 今日外贸英语\n\n【句子】Please quote us on FOB Shanghai basis.\n【释义】请按上海离岸价给我们报价。\n【场景】客户指定贸易术语时，务必确认是 FOB/CIF/EXW，这直接影响运费与风险划分。\n【小技巧】Incoterms 决定谁付运费、谁担风险。FOB=装运港船上交货，CIF=成本+保险+运费。报价前一定问清："Do you prefer FOB or CIF?" 避免后期扯皮。`,
  `📚 今日外贸英语\n\n【句子】We accept T/T 30% deposit and 70% before shipment.\n【释义】我们接受 T/T 30% 定金、70% 发货前付清。\n【场景】付款方式是成交关键，工程机械类大额订单常用 T/T 分期。\n【小技巧】"T/T"（电汇）是主流。常用组合：30% deposit + 70% against B/L copy（见提单副本付尾款），对双方都较安全。若客户要信用证 L/C，需确认即期还是远期。`,
...
const FIN_POOL = [
  `💰 今日理财知识\n\n【知识点一：汇率波动≠焦虑，平常心看人民币】\n【大白话】人民币兑美元短期上下几分钱像天气，谁也猜不准。有美元需求（留学/旅行）可在低位分批换，别梭哈；只持人民币在国内生活，汇率对菜价影响很小。\n【知识点二：基金定投，傻买聪明赚】\n【大白话】熊市攒份额、牛市赚收益。跌时同钱买更多=打折进货，涨时便宜份额发力。别跌时停扣、别涨时追涨。\n【知识点三：资产配置分三份】\n【大白话】随时要用的钱放货币基金；三五年不用的定投宽基指数；保底的钱放债基/定存。三类分开管，别把生活费拿去炒股。`,
  `💰 今日理财知识\n\n【知识点：应急金是你生意的"减震器"】\n【大白话】外贸人收入有季节性、回款有账期，最怕"订单来了但现金断了"。先备足 3-6 个月家庭+基础运营开支的应急金，放货币基金或活期理财，随取随用。\n【建议】应急金到位前，别急着把利润全投出去扩产；它是你能"熬过账期、接住大单"的底气。`,
  `💰 今日理财知识\n\n【知识点：别把美元利润全囤在账上】\n【大白话】做外贸手里常压着美元。人民币升值周期里，美元换回人民币会缩水。散户难择时，建议"分层兑换"：每收到货款，按 30%-50% 结汇，其余留美元等更好点位。\n【建议】用"532"思路：50% 即时结汇保现金流，30% 择机结汇，20% 留作美元资产配置。`,
  `💰 今日理财知识\n\n【知识点：复利是世界第八大奇迹】\n【大白话】每月定投 2000 元、年化 8%，20 年后约 118 万——其中本金仅 48 万，其余是复利。关键在"早开始+不中断+不挪用"。\n【建议】回款到手先划一笔进定投账户再花，强制储蓄比靠自律靠谱。哪怕每月 500 起，时间会替你赚钱。`,
  `💰 今日理财知识\n\n【知识点：通胀悄悄吃掉你的现金】\n【大白话】物价每年涨约 2-3%，钱放银行活期几乎不增值，实际在"贬值"。长期看，纯现金防守反而亏。\n【建议】至少把半年以上不用的钱放进能跑赢通胀的资产（指数基金/债基），别让利润躺在低息账户里睡大觉。`,
  `💰 今日理财知识\n\n【知识点：分散投资，别把鸡蛋放一个篮子】\n【大白话】全仓一只股票/一个行业，涨时爽跌时崩。分散到不同资产（股、债、黄金、现金）能平滑波动，睡得着觉。\n【建议】结合美元收入做"本币+美元"双币种分散；股权类用宽基指数而非单押个股。`,
  `💰 今日理财知识\n\n【知识点：保险是家庭的"止损线"】\n【大白话】理财讲收益，保险讲兜底。一场大病或意外可能清空多年积蓄，一份合适的医疗+意外险，是用小钱买"不破产"的确定性。\n【建议】先给家庭顶梁柱配齐医疗/重疾/意外，再考虑理财型；企业端可关注出口信用保险（中信保），防海外坏账。`,
  `💰 今日理财知识\n\n【知识点：现金流比利润更重要】\n【大白话】账面盈利但回款慢，照样会资金链断裂；"现金流为正"才是企业活命线。很多外贸厂死于"货发出去了钱没回来"。\n【建议】每月做现金排期表，预留 2 个月工资+房租缓冲；大单宁可利润薄也要收预付款，谨防坏账拖垮现金流。`,
  `💰 今日理财知识\n\n【知识点：黄金是动荡期的"压舱石"】\n【大白话】战争、通胀、货币超发时，黄金往往抗跌。它不生息，但能在乱世保住购买力，占总资产 5-10% 较合理。\n【建议】美元+黄金组合能对冲单一货币风险；实物金条/黄金 ETF 都行，别碰高杠杆黄金期货。`,
  `💰 今日理财知识\n\n【知识点：指数基金，普通人最好的朋友】\n【大白话】与其花时间挑个股，不如买一篮子好公司的指数基金（如沪深300、标普500）。长期跟着经济往上走，省心且费用低。\n【建议】用定投方式买宽基指数，忽略短期涨跌；别试图"高抛低吸"，多数人择时反而跑输长期持有。`
];
const CHAT_POOL = [
  ...
  `💬 今日聊天技巧\n\n【技巧】用"背调三问"让冷启动不再尬聊\n【场景】拿到一个陌生存客时\n【怎么做】① 他做什么行业/卖什么产品？② 主要市场在哪？③ 当前用的什么品牌/供应商？\n有了这三点，第一句话就能精准命中痛点，而不是群发"Are you interested?"。`,
  `💬 今日聊天技巧\n\n【技巧】报价后 24 小时内的"软跟进"\n【场景】发完报价客户没回\n【怎么做】不要问"Did you receive my quotation?"（像催债）。改成分享一条与他市场相关的新闻或案例："Saw Chile just approved a new mining project — our dump trucks fit perfectly. Thoughts?" 把跟进变成"给价值"，回复率翻倍。`,
  `💬 今日聊天技巧\n\n【技巧】应对"你的价格比别家高"\n【场景】客户拿竞品比价时\n【怎么做】不硬降，先拆解：质保时长、配件供应、交期、付款条件。"Our price includes 2-year warranty and local spare parts stock — total cost is lower." 把单价战打成总拥有成本战。\n【跨文化提醒】非洲客户重售后网络，强调本地配件仓比降价更打动人。`,
  `💬 今日聊天技巧\n\n【技巧】用"样品策略"筛选真客户\n【场景】分不清谁是真买家时\n【怎么做】真客户愿意为样品付运费甚至样品费；只白嫖报价的大概率是同行。设置"sample available, freight collect"门槛，自然过滤。`,
  `💬 今日聊天技巧\n\n【技巧】节日/斋月问候拉近距离\n【场景】进入客户所在市场的重大节日前\n【怎么做】提前 1-2 周发定制祝福（用当地语言），不夹带广告。"Ramadan Mubarak! May your business flourish." 比群发中文祝福强十倍。`,
  `💬 今日聊天技巧\n\n【技巧】用语音消息破解文字僵局\n【场景】文字聊不动、客户沉默时\n【怎么做】发一条 20-30 秒语音，语速放慢、带笑容。语音有温度，适合解释复杂参数或表达诚意。\n【跨文化提醒】语音用客户母语关键词开头，亲切感拉满；但首次接触慎用长语音，先文字破冰。`,
